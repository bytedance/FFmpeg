...
static void check_url_host(HLSContext *c, struct segment *seg, const char *line) {
    if (!strstr(line, "://")) {
        ...
        seg->need_set_host = 0;
        if (c->headers && !c->headers_without_host) {
            const char* begin = av_strnstr(c->headers, "Host: ", strlen(c->headers));
            if (!begin)
                return;
            ...
            host_position = begin - c->headers;
            const char *end = av_strnstr(begin, "\r\n", strlen(c->headers) - host_position);
            if (end != NULL) {
                host_len = end + 2 - begin;
            } else {
                host_len = sizeof(c->headers) - host_position;
            }

            new_header_len = strlen(c->headers) - host_len;
            new_header_len += 1;
            ...
            if (host_len != 0 && host_position != 0) {
                memcpy(new_header, c->headers, host_position);
                cur_len += host_position;
            }
            if (c->headers)
                memcpy(new_header + cur_len, c->headers + host_position + host_len,  strlen(c->headers) - host_position - host_len);
            *(new_header + new_header_len - 1) = 0x0;

            c->headers_without_host = new_header;

            av_log(c, AV_LOG_INFO, "headers: %s", c->headers);
                          ...
                          struct playlist *pls, AVIOContext *in)
{
    ...
    if (!in) {
        ...
        if (ret < 0)
            return ret;
#else
        char *new_url = c->enable_refresh_by_time
            ? av_asprintf("%s%sbegin_time=%lf", url, av_stristr(url, "?") ? "&" : "?",
                pls->cur_refresh_begin_time * av_q2d(AV_TIME_BASE_Q))
            : av_strdup(url);
        ret = open_in(c, &in, new_url);
        av_freep(&new_url);
        if (ret < 0)
            return ret;
        close_in = 1;
    ...
    while (!avio_feof(in)) {
        ...
        if (av_strstart(line, "#EXT-X-STREAM-INF:", &ptr)) {
            ...
            if (is_segment) {
                ...
                seg->previous_duration = isDiscontinuety ? previous_duration : 0;
                seg->is_discontinuety = isDiscontinuety;
                ...
                av_log(NULL, AV_LOG_TRACE, "no: %d, dis: %d, seg: %lld, %lld, %lld,", pls->n_segments,
...
static int open_input(HLSContext *c, struct playlist *pls, struct segment *seg)
{
    ...
    if (seg->key_type == KEY_NONE) {
        ...
        av_dict_set_int(&opts2, "enable_intertrust_drm", c->enable_intertrust_drm, 0);
        if(c->enable_intertrust_drm) {
            av_dict_set_int(&opts2, "drm_downgrade", c->drm_downgrade, 0);
            av_dict_set_int(&opts2, "drm_aptr", (int64_t)(intptr_t)c->drm_ctx, 0);
            av_dict_set_int(&opts2, "segment_number", seg->segment_number, 0);
        }
