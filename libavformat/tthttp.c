...
static const AVOption options[] = {
    ...
    { "tt_opaque", "set log handle for log", OFFSET(tt_opaque), AV_OPT_TYPE_IPTR, { .i64 = 0 }, INT64_MIN, INT64_MAX, .flags = D|E },
    ...
    { "auto_range_offset", "http range size while read auto range mode", OFFSET(auto_range_offset), AV_OPT_TYPE_INT64, { .i64 = 0 }, 0, INT64_MAX, D },
...
redo:
    ...
    if ((s->http_code == 301 || s->http_code == 302 ||
        ...
        if (redirects++ >= MAX_REDIRECTS){
            av_error(h, AVERROR_HTTP_REDIRECT_COUNT_OUT,"http error");
            return AVERROR(EIO);
        }
        ...
        attempts         = 0;
        location_changed = 0;
        tt_network_info_callback(s->tt_opaque, Is3xxHappen, s->user_flag, NULL);
        goto redo;
    }
...
fail:
    ...
    if (ret != 0) {
        av_error(h, ret, "%s", s->buffer);
	    return ret;
    }
    ret = ff_http_averror(s->http_code, AVERROR(EIO));
	if ( ret == AVERROR(EIO) ) {
		av_error(h, AVERROR_HTTP_DEFAULT_ERROR, "%s",  s->buffer);
	} else {
		av_error(h, ret, "%s", s->buffer);
	}
    return ret;
...
int ff_http_do_new_request(URLContext *h, const char *uri)
{
    ...
    if (!s->location){
        av_warn2(h,AVERROR(ENOMEM),"AVERROR(ENOMEM)");
...
static int http_write_reply(URLContext* h, int status_code)
{
    ...
    case 500:
        ...
        break;
    default:
        av_warn2(h,AVERROR(EINVAL),"AVERROR(EINVAL)");
...
static int http_handshake(URLContext *c)
{
    ...
    av_warn2(ch,AVERROR(EINVAL),"AVERROR(EINVAL)");
    return AVERROR(EINVAL);
}
...
static int http_getc(HTTPContext *s)
{
    ...
    if (s->buf_ptr >= s->buf_end) {
        ...
        if (len < 0) {
            ...
            av_warn2(s,AVERROR(EIO),"AVERROR(EIO)");
            return AVERROR(EIO);
        } else {
            s->recv_size += len;
            s->buf_ptr = s->buffer;
...
static int parse_location(HTTPContext *s, const char *p)
{
    ...
    av_free(s->location);
    s->location = new_loc;
    tt_network_info_callback(s->tt_opaque, IsRedirectURL, 0, new_loc);
...
static int parse_content_encoding(URLContext *h, const char *p)
{
    if (!av_strncasecmp(p, "gzip", 4) ||
        ...
        if (inflateInit2(&s->inflate_stream, 32 + 15) != Z_OK) {
            ...
            av_warn2(s,AVERROR(ENOSYS),"AVERROR(ENOSYS)");
        ...
        av_warn2(h,AVERROR(ENOSYS),"AVERROR(ENOSYS)");
        return AVERROR(ENOSYS);
#endif /* CONFIG_ZLIB */
...
static int parse_cookie(HTTPContext *s, const char *p, AVDictionary **cookies)
{
    ...
    if (!(eql = strchr(p, '='))) {
	    av_warn2(s,AVERROR(EINVAL),"AVERROR(EINVAL)"); 
		return AVERROR(EINVAL);
	}
...
static int cookie_string(AVDictionary *dict, char **cookies)
{
    ...
    *cookies = av_malloc(len);
    if (!*cookies) {
        av_warn2(NULL,AVERROR(ENOMEM),"AVERROR(ENOMEM)");
        return AVERROR(ENOMEM);
    }
    *cookies[0] = '\0';
        ...
        *p  = '\0';
        ...
        if (!av_strcasecmp(tag, "Location")) {
            ...
            *new_location = 1;
            ...
            tt_network_info_callback(s->tt_opaque, IsCDNTimingInfo, (int64_t)s->user_flag, p);
        }
    }
    return 1;
}
...
static int http_read_header(URLContext *h, int *new_location)
{
    ...
    tt_network_info_callback(s->tt_opaque, IsGetResponseHeaders, 0, s->buffer);

    if (s->report_response_headers && len > 0) {
        av_error(h, 0, "response headers: %s\n", headers);
    }
                        ...
                        const char *proxyauth, int *new_location)
{
    ...
    if (s->unlimit_header) {
        ...
        if (s->report_request_headers && len > 0) {
            av_error(h, 0, "request headers: %s\n", headers);
        }
...
static int http_buf_read(URLContext *h, uint8_t *buf, int size)
{
    ...
    if (len > 0) {
        ...
        if (!len && (!s->willclose || s->chunksize == UINT64_MAX) && s->off < target_end) {
            ...
            av_warn2(h,AVERROR(EIO),"AVERROR(EIO)");
            return AVERROR(EIO);
        }
    }
    if (len > 0) {
...
static int http_read_stream(URLContext *h, uint8_t *buf, int size)
{
    ...
    if (   (read_ret  < 0 && s->reconnect  && read_ret != AVERROR_EXIT && (!h->is_streamed || s->reconnect_streamed) && target_end > 0 && s->off < target_end)
        ...
        if (s->reconnect_delay > s->reconnect_delay_max){
            av_warn2(h,AVERROR(EIO),"AVERRR(EIO)");
            return AVERROR(EIO);
        }
#endif
...
static int store_icy(URLContext *h, int size)
{
    ...
    if (s->icy_metaint < s->icy_data_read) {
	    av_warn2(h,AVERROR_INVALIDDATA,"AVERROR_INVALIDDATA");
...
static int64_t http_seek_internal(URLContext *h, int64_t off, int whence, int force_reconnect)
{
    ...
    if ( ret < 0) {
        av_warn2(h,ret,"reconnect:%d delay_time:%d,fail:%d", reconnect_index, reconnect_delay_time);
        av_dict_free(&options);
...
#if CONFIG_TTHTTP_PROTOCOL
#if CONFIG_HTTP_PROTOCOL
#error tthttp and http protocol file conflict
#endif
HTTP_CLASS(http);

const URLProtocol ff_tthttp_protocol = {
    .name                = "http",
    .url_open2           = http_open,
...
#endif /* CONFIG_TTHTTP_PROTOCOL */

#if CONFIG_TTHTTPS_PROTOCOL
#if CONFIG_HTTPS_PROTOCOL
#error tthttps and https protocol file conflict
#endif
HTTP_CLASS(https);

const URLProtocol ff_tthttps_protocol = {
    .name                = "https",
    .url_open2           = http_open,
    .url_read            = http_read,
    .url_write           = http_write,
...
#endif /* CONFIG_TTHTTPQ_PROTOCOL */

#if CONFIG_TTHTTPPROXY_PROTOCOL
#if CONFIG_HTTPPROXY_PROTOCOL
#error tthttpproxy and httpproxy protocol file conflict
#endif
...
const URLProtocol ff_tthttpproxy_protocol = {
    .name                = "httpproxy",
...
#endif /* CONFIG_TTHTTPPROXY_PROTOCOL */
