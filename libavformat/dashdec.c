...
static int ishttp(const char *url)
{
    const char *protocol_name = url ? avio_find_protocol_name(url) : NULL;
    return protocol_name ? av_strstart(protocol_name, "http", NULL) : 0;
}
...
static char *get_cenc_default_kid(const char *kid)
{
    ...
    for (i = 0; i < n_baseurl_nodes; ++i) {
        if (baseurl_nodes[i] &&
            ...
            if (text) {
                ...
                ff_make_absolute_url2(tmp_str_2, max_url_size, tmp_str, text);
                                         ...
                                         xmlNodePtr adaptionset_contentprotection_node)
{
    ...
    if (type == AVMEDIA_TYPE_UNKNOWN) {
        ...
        c->max_url_size = aligned(c->max_url_size  + strlen(rep_id_val) + rep_bandwidth_len);
            ...
            if (representation_segmentbase_node) {
                ...
                if (ishttp(url)) {
                    ...
                    if (segmentbase_initialization_node) {
                        initialization_range_val = xmlGetProp(segmentbase_initialization_node, "range");
                    }
                    index_range_val = xmlGetProp(representation_segmentbase_node, "indexRange");
                }
            }
...
static int64_t find_nearest_fragment(struct representation *pls, int64_t pos_msec) {
    int i = 0;
    int j = 0;
    int64_t duration = 0;
...
static int64_t calc_cur_seg_no(AVFormatContext *s, struct representation *pls)
{
    ...
    if (c->is_live) {
        ...
        num = find_nearest_fragment(pls, c->start_time);
        if (pls->cur_seq_no != 0) {
...
static int open_input(DASHContext *c, struct representation *pls, struct fragment *seg)
{
    ...
    char *url = NULL;
    int ret = 0;
    int64_t offset = seg->url_offset > 0 ? seg->url_offset : 0;
    ...
    if (offset >= 0) {
        ...
        av_dict_set_int(&opts, "offset", offset, 0);
    }
    if (seg->size >= 0) {
        av_dict_set_int(&opts, "end_offset", offset + seg->size, 0);
    }

    ff_make_absolute_url2(url, c->max_url_size, c->base_url, seg->url);
    
    av_log(pls->parent, AV_LOG_VERBOSE, "DASH request for url '%s', offset %"PRId64", playlist %d\n",
           url, offset, pls->rep_idx);
    ret = open_url(pls->parent, &pls->input, url, c->avio_opts, opts, NULL);
    if (ret < 0) {
        goto cleanup;
    } else if (pls->is_segmentbase) {
        pls->input->pos += offset;
    }

cleanup:
    av_free(url);
    av_dict_free(&opts);
    pls->cur_seg_offset = offset;
    pls->cur_seg_size = seg->size;
    return ret;
}

static int check_init_section(uint8_t *sec_buf) {
    if (!sec_buf) {
        av_log(NULL, AV_LOG_ERROR, "check init section failed, sec_buf is null\n");
        return AVERROR_INVALIDDATA;
    }
    if (!av_stristr(sec_buf, "<MPD")) {
        return 0;
    }
    if (av_stristr(sec_buf, "dash:profile:isoff-on-demand:2011") ||
        av_stristr(sec_buf, "dash:profile:isoff-live:2011") ||
        av_stristr(sec_buf, "dash:profile:isoff-live:2012") ||
        av_stristr(sec_buf, "dash:profile:isoff-main:2011") ||
        av_stristr(sec_buf, "dash:profile")) {
...
static int update_init_section(struct representation *pls)
{
    ...
    if (ret == sec_size)
        avio_shutdown(pls->input, AVIO_FLAG_STOP | AVIO_FLAG_REUSE);
#endif
    ff_format_io_close(pls->parent, &pls->input);
    ...
    ret = check_init_section(pls->init_sec_buf);
    if (ret < 0) {
        return ret;
    }
...
static int reopen_demux_for_component(AVFormatContext *s, struct representation *pls)
{
    ...
    if (c->drm_ctx) {
        ...
        av_dict_set_int(&in_fmt_opts, "drm_downgrade", c->drm_downgrade, 0);
        av_dict_set_int(&in_fmt_opts, "drm_aptr", (int64_t)(intptr_t) c->drm_ctx, 0);
    }
#endif
...
static int open_demux_for_component(AVFormatContext *s, struct representation *pls)
{
    ...
    for (i = 0; i < pls->ctx->nb_streams; i++) {
        ...
        if (!st) {
            av_log(s,AV_LOG_ERROR,"st is null");
            ret = AVERROR(ENOMEM);
            goto fail;
        }
        st->id = i;
        ...
        // copy side data
        for (int j = 0; j < ist->nb_side_data; j++) {
            const AVPacketSideData *sd_src = &ist->side_data[j];
            uint8_t *dst_data;
...
static int probe_stream_index(AVFormatContext *s, int bitrate, int type)
{
    DASHContext *c = s->priv_data;
    struct representation **reps = NULL;
    int rep_count = 0;
    int i;
    int index = 0;
    int min = INT_MAX;
    if (type == AVMEDIA_TYPE_VIDEO) {
        reps = c->videos;
        rep_count = c->n_videos;
    } else if (type == AVMEDIA_TYPE_AUDIO) {
        reps = c->audios;
        rep_count = c->n_audios;
    }
    for (i = 0; i < rep_count; i++) {
        struct representation *rep = reps[i];
        int diff = bitrate - rep->bandwidth;
...
static int dash_read_header(AVFormatContext *s)
{
...
#if defined(__APPLE__)
    if (!(s->flags & AVFMT_FLAG_CUSTOM_IO))
        avio_shutdown(s->pb, AVIO_FLAG_STOP | AVIO_FLAG_REUSE);
#endif
    ...
    if (c->n_videos) {
        struct representation *cur_video = NULL;
        index = probe_stream_index(s,c->cur_video_bitrate,AVMEDIA_TYPE_VIDEO);
        c->is_init_section_common_video = is_common_init_section_exist(c->videos, c->n_videos);
        c->next_video = index;
        c->cur_video = index;
        cur_video = c->videos[c->cur_video];
        ret = open_demux_for_component(s, cur_video);
        if (ret)
            goto fail;
        cur_video->stream_index = c->stream_count++;
        
        if (c->is_init_section_common_video) {
            for (i = 0; i < c->n_videos; i++) {
                if (i != c->cur_video) {
                    copy_init_section(c->videos[i], cur_video);
                }
            }
        }
    }
    ...
    if (c->n_audios) {
        struct representation *cur_audio = NULL;
        index = probe_stream_index(s,c->cur_audio_bitrate,AVMEDIA_TYPE_AUDIO);
        c->is_init_section_common_audio = is_common_init_section_exist(c->audios, c->n_audios);
        c->next_audio = index;
        c->cur_audio = index;
        cur_audio = c->audios[c->cur_audio];
        ret = open_demux_for_component(s, cur_audio);
        if (ret)
            goto fail;
        cur_audio->stream_index = c->stream_count++;
        
        if (c->is_init_section_common_audio) {
            for (i = 0; i < c->n_audios; i++) {
                if (i != c->cur_audio) {
                    copy_init_section(c->audios[i], cur_audio);
                }
            }
        }
    }
...
static int check_seek(struct representation *pls, int stream_index, int64_t seek_pos) {
    AVStream *stream;
    int64_t start_pos, end_pos;
    double timebase;
    if (!pls || !pls->ctx || !pls->is_need_check_seek || pls->seek_retry_error != 0) {
        goto fail;
    }
    if (stream_index < 0) {
        stream_index = av_find_default_stream_index(pls->ctx);
        if (stream_index < 0) {
            av_log(NULL, AV_LOG_ERROR, "find stream index error, index:%d\n", stream_index);
            goto fail;
        }
    }
    stream = pls->ctx->streams[stream_index];
    if (!stream) {
        av_log(NULL, AV_LOG_ERROR, "cur_no:%"PRId64" stream is null\n", pls->cur_seq_no);
        goto fail;
    } else if (stream->nb_index_entries <= 1) {
        av_log(NULL, AV_LOG_ERROR, "check seek fail sample count:%d\n", stream->nb_index_entries);
        goto fail;
    }
    start_pos = av_read_timestamp2(pls->ctx, stream_index, 0);
    end_pos = av_read_timestamp2(pls->ctx, stream_index, stream->nb_index_entries - 1);
    if (start_pos == AV_NOPTS_VALUE || end_pos == AV_NOPTS_VALUE) {
        av_log(NULL, AV_LOG_ERROR, "cur_no:%"PRId64" start_pos or end_pos is AV_NOPTS_VALUE\n",
            pls->cur_seq_no);
        goto fail;
    }
    timebase = av_q2d(stream->time_base) * 1000;
    start_pos *= timebase;
    end_pos *= timebase;
    if (seek_pos >= start_pos && seek_pos <= end_pos) {
        pls->seek_retry_offset = 0;
    } else if (seek_pos < start_pos && pls->cur_seq_no > pls->first_seq_no) {
        pls->seek_retry_offset += FFMAX((seek_pos - start_pos) / (end_pos - start_pos) - 1,
            pls->first_seq_no - pls->cur_seq_no);
    } else if (seek_pos > end_pos && pls->cur_seq_no < pls->last_seq_no) {
        pls->seek_retry_offset += FFMIN((seek_pos - end_pos) / (end_pos - start_pos) + 1,
            pls->last_seq_no - pls->cur_seq_no);
    } else {
        pls->seek_retry_offset = 0;
    }
    av_log(NULL, AV_LOG_DEBUG, "type:%d "
        "seek_pos:%"PRId64" start_pos:%"PRId64" end_pos:%"PRId64" "
        "cur_no:%"PRId64" first_no:%"PRId64" last_no:%"PRId64" "
        "seek_retry_offset:%"PRId64"\n",
        pls->type, seek_pos, start_pos, end_pos, 
        pls->cur_seq_no, pls->first_seq_no, pls->last_seq_no,
        pls->seek_retry_offset);
    return pls->seek_retry_offset != 0 ? EAGAIN : 0;
fail:
    if (pls) {
        pls->seek_retry_error = AVERROR_UNKNOWN;
        if (pls->seek_retry_offset != 0) {
            pls->seek_retry_offset = 0;
            return EAGAIN;
        }
    }
...
static int dash_seek(AVFormatContext *s, struct representation *pls, int64_t seek_pos_msec, int flags, int need_open_pre_seq);
...
static int dash_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (c->n_videos && !c->radio_mode) {
        struct representation *pls = c->videos[c->cur_video];
        if (!cur || pls->cur_timestamp < mints) {
            cur = pls;
            mints = pls->cur_timestamp;
            // av_log(s,AV_LOG_INFO,"read video pts:%lld",mints);
        }
    }

    if (c->n_audios) {
        struct representation *pls = c->audios[c->cur_audio];
        if (!cur || pls->cur_timestamp < mints) {
            cur = pls;
            mints = pls->cur_timestamp;
            // av_log(s,AV_LOG_INFO,"read audio pts:%lld",mints);
        }
    }


    if (!c->radio_mode && c->radio_mode_seek != -1) {
        struct representation *pls = c->videos[c->cur_video];
        int64_t seek_target = c->radio_mode_seek;
        c->radio_mode_seek = -1;
        dash_seek(s, pls, seek_target, AVSEEK_FLAG_BACKWARD, 0);
    }
    ...
    while (!ff_check_interrupt(c->interrupt_callback) && !ret) {
        if (!cur->is_restart_needed) {
            ...
            if (ret >= 0) {
                ...
                cur->cur_timestamp = av_rescale(pkt->pts, (int64_t)cur->ctx->streams[0]->time_base.num * 1000, cur->ctx->streams[0]->time_base.den);
                if (c->n_videos && c->n_audios) {
                    pkt->stream_index = cur->stream_index;
                    if (cur->type == AVMEDIA_TYPE_VIDEO) {
                        if ((pkt->flags & AV_PKT_FLAG_KEY) != 0) {
                            int64_t probe_bitrate = -1;
                            if (c->abr) {
                                getABRStrategyCtx *abr = (getABRStrategyCtx *)c->abr;
                                probe_bitrate = abr->caculateBitrate(c->aptr, cur->bandwidth);
                            }
                            if (probe_bitrate >= 0) {
                                c->cur_video_bitrate = probe_bitrate;
                                av_log(s,AV_LOG_VERBOSE,"probe bitrate:%lld",probe_bitrate);
                            }
                            c->next_video = probe_stream_index(s,c->cur_video_bitrate,AVMEDIA_TYPE_VIDEO);
                            if (c->next_video != c->cur_video) {
                                int seq_no = cur->cur_seq_no;
                                int64_t seek_pos_msec = cur->cur_timestamp;
                                av_log(s, AV_LOG_VERBOSE, "change stream from:%d, to:%d", c->cur_video, c->next_video);
                                if (cur->input) {
                                    ff_format_io_close(cur->parent,&cur->input);
                                }
                                c->cur_video = c->next_video;
                                cur = c->videos[c->cur_video];
                                cur->cur_timestamp = 0;
                                cur->cur_seq_no = seq_no;
                                cur->seek_pos = seek_pos_msec;
                                cur->seek_flags = AVSEEK_FLAG_BACKWARD;
                                cur->seek_retry_error = 0;
                                cur->seek_retry_offset = 0;
                                cur->is_seeking = 1;
                                cur->is_restart_needed = 1;
                                c->need_inject_sidedata = 1;
                                goto label_restart;
                            }
                            if (c->need_inject_sidedata) {
                                c->need_inject_sidedata = 0;
                                extradata_size = cur->assoc_stream->codecpar->extradata_size;
                                extradata = cur->assoc_stream->codecpar->extradata;
                ...
                //            av_log(s,AV_LOG_INFO,"read pkt pts:%lld,stream_index:%d,seq_no:%lld",cur->cur_timestamp,pkt->stream_index,cur->cur_seq_no);
                return 0;
            }
        }
    label_restart:
        if (cur->is_restart_needed) {
            av_log(s, AV_LOG_VERBOSE, "cur type:%d is seeking, seek_pos:%"PRId64", cur_seq_no:%"PRId64", ret:%d\n",
                cur->type, cur->seek_pos, cur->cur_seq_no, ret);
            cur->cur_timestamp = 0;
...
#if defined(__APPLE__)
                if (!cur->is_seeking) {
                    avio_shutdown(cur->input,AVIO_FLAG_STOP | AVIO_FLAG_REUSE);
                }
#endif
                ff_format_io_close(cur->parent, &cur->input);
            }
            if (cur->is_seeking) {
                cur->cur_seq_no = find_nearest_fragment(cur, cur->seek_pos) + cur->seek_retry_offset;
                av_log(s, AV_LOG_VERBOSE, "cur type:%d is seeking, seek_pos:%"PRId64", cur_seq_no:%"PRId64", seek_retry_offset:%"PRId64"\n",
                    cur->type, cur->seek_pos, cur->cur_seq_no, cur->seek_retry_offset);
            } else {
                if (cur->type == AVMEDIA_TYPE_VIDEO) {
                    int64_t probe_bitrate = -1;
                    if (c->abr) {
            ...
            if (!cur->is_opened) {
                av_log(s,AV_LOG_VERBOSE,"reopen cur type:%d",cur->type);
                cur->cur_seg_offset = 0;
                cur->init_sec_buf_read_offset = 0;
                ret = open_demux_for_component(s, cur);
                if (ret) {
                    break;
                }
                cur->stream_index = c->stream_count++;
                av_program_add_stream_index(s, 0, cur->stream_index);
                cur->assoc_stream = s->streams[cur->stream_index];
                cur->assoc_stream->start_time = 0;
                if (cur->bandwidth > 0)
                    av_dict_set_int(&cur->assoc_stream->metadata, "variant_bitrate", cur->bandwidth, 0);
                if (cur->id[0])
                    av_dict_set(&cur->assoc_stream->metadata, "id", cur->id, 0);
                if (cur->is_seeking) {
                    if (check_seek(cur, -1, cur->seek_pos) == EAGAIN) {
                        goto label_restart;
                    }
                    av_seek_frame(cur->ctx, -1, cur->seek_pos * 1000, cur->seek_flags);
                    cur->is_seeking = 0;
                    cur->seek_pos = -1;
                    cur->seek_flags = -1;
                }
...
static int dash_seek(AVFormatContext *s, struct representation *pls, int64_t seek_pos_msec, int flags, int need_open_pre_seq)
{
    DASHContext *c = s->priv_data;
    int stream_change = 0;
    int64_t cur_seq_no = 0;

    av_log(pls->parent, AV_LOG_VERBOSE, "DASH seek pos[%"PRId64"ms], playlist %d%s\n",
            seek_pos_msec, pls->rep_idx, need_open_pre_seq ? " (need_open_pre_seq)" : "");

    if (pls->type == AVMEDIA_TYPE_VIDEO) {
        c->need_inject_sidedata = 1;
        av_log(pls->parent, AV_LOG_VERBOSE, "switch stream from %d to %d",c->cur_video,c->next_video);
        if (c->next_video != c->cur_video) {
            struct representation *last = NULL;
            stream_change = 1;
            last = c->videos[c->cur_video];
            if (last->input) {
                ff_format_io_close(last->parent,&last->input);
            }
            c->cur_video = c->next_video;
        }
    } else if (pls->type == AVMEDIA_TYPE_AUDIO) {
        if (c->next_audio != c->cur_audio) {
            struct representation *last = NULL;
            stream_change = 1;
            last = c->audios[c->cur_audio];
            if (last->input) {
                ff_format_io_close(last->parent,&last->input);
            }
            c->cur_audio = c->next_audio;
        }
    }
    
    // single fragment mode
    if (pls->n_fragments == 1 && pls->ctx && !stream_change) {
        pls->cur_timestamp = 0;
    ...
    cur_seq_no = find_nearest_fragment(pls, seek_pos_msec);
    av_log(pls->parent, AV_LOG_VERBOSE, "DASH seek pos[%"PRId64"ms], playlist %d, cur_seq_no:%"PRId64", pls->cur_seq_no:%"PRId64"\n",
           seek_pos_msec, pls->rep_idx, cur_seq_no, pls->cur_seq_no);
    if (cur_seq_no == pls->cur_seq_no - 1 && !need_open_pre_seq) {
        cur_seq_no = pls->cur_seq_no;
    }
    
    if (pls->cur_seq_no != cur_seq_no || !pls->is_opened || !pls->ctx || stream_change) {
        pls->cur_timestamp = 0;
        pls->is_seeking = 1;
        pls->seek_pos = seek_pos_msec;
        pls->seek_flags = flags;
        pls->seek_retry_error = 0;
        pls->seek_retry_offset = 0;
        pls->is_restart_needed = 1;
        return 0;
    }
    if (pls->ctx) {
        av_seek_frame(pls->ctx, -1, seek_pos_msec * 1000, flags);
    }
...
{
    int ret = 0;
    int need_open_pre_seq = 1;
    DASHContext *c = s->priv_data;
    struct representation *video_pls = NULL;
    struct representation *audio_pls = NULL;
    AVStream *cur_stream = NULL;
    int cur_stream_index = -1;
    int64_t video_serached_msec = AV_NOPTS_VALUE;
    int64_t seek_pos_msec = av_rescale_rnd(timestamp, 1000, s->streams[stream_index]->time_base.den,
    ...
    if (!ret && c->n_videos && !c->radio_mode) {
        c->next_video = probe_stream_index(s, c->cur_video_bitrate, AVMEDIA_TYPE_VIDEO);
        ret = dash_seek(s, c->videos[c->next_video], seek_pos_msec, flags, 1);
        video_pls = c->videos[c->next_video];
    }

    if (!ret && video_pls && video_pls->ctx && !video_pls->is_restart_needed && !video_pls->is_segmentbase && !c->radio_mode) {
        cur_stream_index = av_find_default_stream_index(video_pls->ctx);
        if (cur_stream_index >= 0) {
            int cur_video_index = -1;
            int64_t video_serached_pos = AV_NOPTS_VALUE;
            cur_stream = video_pls->ctx->streams[cur_stream_index];
            cur_video_index = av_index_search_timestamp(cur_stream, timestamp, flags);
            if (cur_video_index >= 0) {
                video_serached_pos = av_read_timestamp2(video_pls->ctx, cur_stream_index, cur_video_index);
            } else {
                int64_t start_pos = av_read_timestamp2(video_pls->ctx, cur_stream_index, 0);
                int64_t end_pos = av_read_timestamp2(video_pls->ctx, cur_stream_index, cur_stream->nb_index_entries - 1);
                if (start_pos != AV_NOPTS_VALUE && timestamp < start_pos) {
                    video_serached_pos = start_pos;
                } else if (end_pos != AV_NOPTS_VALUE && timestamp > end_pos) {
                    video_serached_pos = end_pos;
                } else {
                    video_serached_pos = AV_NOPTS_VALUE;
                }
            }
            if (video_serached_pos != AV_NOPTS_VALUE) {
                video_serached_msec =
                    av_rescale(video_serached_pos, (int64_t) cur_stream->time_base.num * 1000, cur_stream->time_base.den);
            }
            av_log(NULL, AV_LOG_VERBOSE, "video serached: %" PRId64 "msec index:%d\n", video_serached_msec, cur_video_index);
        }
    }

    if (!ret && c->n_audios) {
        c->next_audio = probe_stream_index(s, c->cur_audio_bitrate, AVMEDIA_TYPE_AUDIO);
        audio_pls = c->audios[c->next_audio];
        if (audio_pls && audio_pls->ctx && video_serached_msec != AV_NOPTS_VALUE) {
            cur_stream_index = av_find_default_stream_index(audio_pls->ctx);
            if (cur_stream_index >= 0) {
                int64_t start_pos = av_read_timestamp2(audio_pls->ctx, cur_stream_index, 0);
                cur_stream = audio_pls->ctx->streams[cur_stream_index];
                if (start_pos != AV_NOPTS_VALUE) {
                    int64_t start_msec =
                        av_rescale(start_pos, (int64_t) cur_stream->time_base.num * 1000, cur_stream->time_base.den);
                    if ((start_msec < video_serached_msec) || (start_msec - video_serached_msec) < 100) {
                        need_open_pre_seq = 0;
                    }
                }
            }
        ...
        seek_pos_msec = (!video_pls || video_pls->is_restart_needed || video_serached_msec == AV_NOPTS_VALUE) ? seek_pos_msec : video_serached_msec;
        ret = dash_seek(s, c->audios[c->next_audio], seek_pos_msec, flags, need_open_pre_seq);
    }
...
static const AVOption dash_options[] = {
    ...
    {"cur_video_bitrate", "the bitrate of video stream",
        ...
        {.i64 = 0},
        0, INT_MAX, D},
    {"cur_audio_bitrate", "the bitrate of audio stream",
        OFFSET(cur_audio_bitrate), AV_OPT_TYPE_INT,
        {.i64 = 0},
        0, INT_MAX, D},
    {"start_time", "the start time msec of video",
        OFFSET(start_time), AV_OPT_TYPE_INT,
        {.i64 = 0},
        0, INT_MAX, D},
    ...
    { "radio_mode_seek", "radio_mode_seek (video seek position after audio mode end)", OFFSET(radio_mode_seek), AV_OPT_TYPE_INT64, { .i64 = -1 }, INT64_MIN, INT64_MAX, D },
    {NULL}
};
